...
class GalData(Dataset):
    ...
    def load(self, **kwargs):
        ...
        self.__dict__.update(kwargs)
        if validate:
            train, trainlabs, _ = self.readfiles('train_filter', [7])
            test, testlabs, self.testinds = self.readfiles('train_filter', [8])
        else:
            train, trainlabs, _ = self.readfiles('train_filter', range(1, 9))
            test, testlabs, self.testinds = self.readfiles('test_filter', [9, 10])
